...
- A handful of rules that duplicated stylelint's own built-in checks (e.g. `max-nesting-depth`, `selector-max-specificity`, `block-no-empty`, `no-*-vendor-prefix`) were removed in favor of using stylelint core directly — no need to maintain a second implementation of something stylelint already does. The equivalent stylelint core rules are still included in the presets that previously shipped our version, so extending a preset doesn't lose coverage — see the tables above for which rules are `(stylelint core)`.
